"""Long-lived parity driver for the frozen Python TreeSitterTsJsChunker.

Sibling of run_chunk_python_ref.py — same JSONL stdin/stdout protocol, but drives the TS/JS chunker
(codemaster.chunking.treesitter_tsjs.TreeSitterTsJsChunker) so the TypeScript port can be proven
byte-equal against the source-of-truth. One interpreter, many requests: read JSONL on stdin and emit
one JSON line per request on stdout. Runs under the frozen submodule's venv with cwd at
vendor/codemaster-py so `import codemaster` resolves the source-of-truth.

One op kind:

    {"id": "...", "op": "chunk_tsjs", "path": "x.ts", "body_b64": "<base64 of raw file BYTES>",
     "hunk_ranges": [[start, end], ...]}
        Decodes body_b64 → bytes, constructs the frozen TreeSitterTsJsChunker(), awaits
...
from codemaster.chunking.treesitter_tsjs import TreeSitterTsJsChunker

_CHUNKER = TreeSitterTsJsChunker()
